...
#Above is where the db variables that we created come into play. 
...
#With the preceding code below, anytime we want to perform a CRUD operation using SQLAlchemy, we need to establish a temporary session 
# connection with the Session class. Within this db session object, we use the add_all() method and 
# the User model to create several new users.

db = Session()

# insert users // Note that db.add_all() alone doesn't change the database. In fact, it only preps the SQL queries. 
db.add_all([
...
#If you don't need to make any other database transactions, you can close the session connection by calling db.close().
db.close()